...
The following instructions are relevant to ns-3.37 release and Windows 10.
...
This is the environment we are going to use to get the MinGW64 toolchain for ns-3.

Most of the MinGW64 toolchain tested came from `Msys2`_.
It needs to be installed before proceeding.

Msys2 will be installed by default in a Msys64 directory containing multiple toolchains,
including: MinGW32, MinGW64, Clang64, UCRT, etc. The only tested one was MinGW64.

The next required step is adding the binaries directories from MinGW64 and Msys2 to the
PATH environment variable. This can be accomplished via the GUI (search for system environment variable),
...
Note: if the MinGW64 binary directory doesn't precede the Windows/System32 directory, documentation build
will fail since Windows has a conflicting ``convert`` command (FAT-to-NTFS).

Accessing the MinGW64 shell
+++++++++++++++++++++++++++

This is how we can get to the MinGW64 shell and use the Pacman package manager.
...
.. sourcecode:: console
    ...
    /c/ns-3-dev/ MINGW64$ pacman -S mingw-w64-x86_64-toolchain mingw-w64-x86_64-cmake mingw-w64-x86_64-ninja mingw-w64-x86_64-grep mingw-w64-x86_64-sed
...
The second part is the `Microsoft MPI`_ executors (mpiexec, mpirun),
which is distributed as a installable (``msmpisetup.exe``).

After installing it, we need to add it to both Windows and MinGW64 shells, if
we want to call MPI programs from both shells.

.. sourcecode:: console

    C:\> setx PATH "%PATH%;C:\Program Files\Microsoft MPI\Bin" /m
    C:\ns-3-dev\> set MSYSTEM MINGW64
    C:\ns-3-dev\> bash
...
After installing, we export the environment variables once again
...
The figures typically are written in dia.
...
Sphinx can be installed via Msys2:
...
After installing it, or unzipping the package (example), add Dia/bin to the PATH.
...
At this point, we can either clone ns-3:
...
Or access the ~/mingw64_test/ from the host machine, where the Vagrantfile resides, by accessing
...
Enable essential and disable unnecessary services
=================================================
