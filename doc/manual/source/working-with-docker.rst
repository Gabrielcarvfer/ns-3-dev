...
on YAML files, which specify a container the job will run on.

A container is a light virtualization tool that allows an user to use user-space
tools from different OSs on top of the same kernel. This drastically cuts the
...
between a host and a guest OS via an hypervisor.

The most common type of containers are applications containers, where
:ref:`Docker <Docker containers>` reigns supreme as the most popular solution.
...
Note: if you are a government employee, or use Docker on a company of more than
250 employees OR that makes more than $10 million in annual revenue, you or your
employer need to pay a subscription fee. If you are not certain if you fit one
of these criteria, you can use :ref:`Podman <Podman containers>` as a drop-in
replacement.

`Docker`_ popularized containers and made them ubiquitous in continuous integration
due to its ease of use. This section will consolidate the basics on how to work
with Docker, which can also be used with Podman.
...
Docker is usually setup (e.g. via system package managers or their official
installers) in root mode, requiring frequent use of administrative
permissions/sudo, which is necessary for some services but not for most. You
can install Docker in `rootless mode`_.
...
A ``Dockerfile`` defines how an image is setup to prepare it to run the
containerized applications/services. These files are composed of
...
* RUN: run commands to install dependencies and setup the service
* ENTRYPOINT: program to be started with the container

Here is one example of an ``Dockerfile`` that can be used to debug an ns-3
...
include docker.io, quay.io, GitHub, GitLab, or you can have your own
self-hosted option.
...
One example on how this can be done is shown below:
...
When building toolchain containers to work in projects, we typically don't
want to copy the projects themselves into the container. So we need to pass
an empty directory to Docker.

We also need to specify a tag to our image, not to end up having to use
randomly assigned names to use them later.
...
their images come from, instead of risking pulling image from different
...
This is for demonstration purposes only. For toolchain containers,
we use the ``run`` command instead. You can jump directly to the
`intended way to use toolchain containers`_, or continue reading
in case you want to learn a bit more about Docker.
...
As it can be seen, our container was started, executed bash, but it
is currently in a non-interactive session, so it doesn't wait for
the user and stopped the service. Which then allowed the container
to be turned off.
...
With this, we can point the real ns-3-dev directory and work as usual.
...
In case you don't want to push your image to public servers
and host your own registry, you can run your own registry
inside a container.
...
You can install it and setup an alias ``alias docker=podman`` in ``~/.bashrc``.

To get the same exact experience you have with docker, you might want to
