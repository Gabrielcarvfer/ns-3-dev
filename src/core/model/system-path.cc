...
Exists(const std::string path)
{
    ...
    // Delegate to std::filesystem. The previous implementation derived the
    // directory part of the path and searched it for the final component;
    // for a bare relative filename (no separator) the derived directory was
    // the empty string, which does not exist, so Exists() wrongly returned
    // false for files in the current working directory (issue #781). It also
    // mishandled trailing separators and file-vs-directory distinctions.
    // The error_code overload is used so this never throws, matching the
    // previous no-throw behavior.
    std::error_code ec;
