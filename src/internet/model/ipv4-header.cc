...
Ipv4Header::SetLooseSourceRoute(const std::vector<Ipv4Address>& route)
{
    ...
    // The option space is bounded by the largest header the IHL field can
    // express: 60 bytes, of which 40 hold options, i.e. 9 addresses beside
    // the type, length and pointer octets (RFC 791)
    NS_ABORT_MSG_IF(route.size() > 9,
                    "A loose source route holds at most 9 addresses, not " << route.size());
    m_sourceRoute = route;
...
Ipv4Header::Print(std::ostream& os) const
{
    ...
    os << "tos 0x" << std::hex << m_tos << std::dec << " "
       ...
       << "flags [" << flags << "] "
       << "length: " << (m_payloadSize + m_headerSize) << " " << m_source << " > " << m_destination;
    if (!m_sourceRoute.empty())
    {
        os << " lsrr [";
        for (const auto& hop : m_sourceRoute)
        {
            os << " " << hop;
        }
        os << " ] pointer " << static_cast<uint16_t>(m_sourceRoutePointer);
    }
}
...
Ipv4Header::Serialize(Buffer::Iterator start) const
{
    ...
    if (optionsLength > 0)
    {
        ...
        // The option is 3 bytes past a word boundary, so a single End of
        // Option List byte brings the options to the next one
        for (uint16_t pad = 3 + 4 * m_sourceRoute.size(); pad < optionsLength; ++pad)
        {
            i.WriteU8(IPV4_OPTION_END);
        }
    }

    if (m_calcChecksum)
    {
        // The checksum covers the options as well as the fixed fields
        i = start;
        uint16_t checksum = i.CalculateIpChecksum(headerLength);
