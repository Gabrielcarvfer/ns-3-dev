...
TcpHeader::Deserialize(Buffer::Iterator start)
{
    ...
    while (optionLen)
    {
        ...
        // The experimental option kinds of RFC 6994 (253 and 254) may
        // legitimately appear more than once, distinguished by an identifier
        // inside the option
        constexpr uint8_t EXPERIMENTAL_KIND_1 = 253;
        constexpr uint8_t EXPERIMENTAL_KIND_2 = 254;
        if (kind != TcpOption::END && kind != TcpOption::NOP && kind != EXPERIMENTAL_KIND_1 &&
            kind != EXPERIMENTAL_KIND_2 && HasOption(kind))
        {
            // A repeated option would be shadowed by the first instance when
            // looked up with GetOption (see issue #940)
            NS_LOG_ERROR("Duplicate TCP option " << static_cast<int>(kind)
                                                 << "; options discarded");
            m_malformed = true;
            break;
        }
