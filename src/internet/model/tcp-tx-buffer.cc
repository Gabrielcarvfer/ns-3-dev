...
TcpTxBuffer::DiscardUpTo(const SequenceNumber32& seq, const Callback<void, TcpTxItem*>& beforeDelCb)
{
    ...
    if (!m_sentList.empty())
    {
        ...
        if (head->m_sacked)
        {
            ...
            // The RFC 6675 IsLost predicate: more than (dupThresh - 1) * SMSS
            // sacked bytes tell a loss apart from reordering
            bool lossInferred = m_sackedOut > (m_dupAckThresh - 1) * m_segmentSize;
            head->m_sacked = false;
