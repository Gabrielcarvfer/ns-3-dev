...
/*
 * KNOWN-FAILING regression test for ns-3 issue #825.
 *
 * This is the test code for the IP-layer "no route -> ICMP unreachable" gap.
 *
 * Gap description:
 *   Ipv4L3Protocol::RouteInputError() (src/internet/model/ipv4-l3-protocol.cc)
 *   contains an explicit
 *
 *       // \todo Send an ICMP no route.
 *
 *   When a router receives a packet that it must forward but for which it has
 *   no matching route, the routing input callback fails and the packet is
 *   silently dropped via m_dropTrace(..., DROP_ROUTE_ERROR, ...). No ICMP
 *   Destination Unreachable (type 3) message is generated back toward the
 *   source. As a consequence, a Ping (or any other) application on the source
 *   never sees the expected ICMP error (issue #825).
 *
 * What this test does:
 ...
 *
 * Observed behaviour on current master (pre-fix):
 *   R silently drops the un-routable packet and sends nothing back, so A
 *   receives 0 ICMP errors. This test therefore FAILS BY DESIGN on the current
 *   tree and will start to PASS once Ipv4L3Protocol::RouteInputError() is fixed
 *   to generate an ICMP net-unreachable message on a no-route forwarding
 *   failure.
 */
 ...
 * @ingroup tests
 *
 * @brief KNOWN-FAILING regression test for issue #825: a router that cannot
 *        forward a packet (no matching route) must reply to the source with an
 *        ICMP Destination Unreachable (net-unreachable) message.
 *
 * Topology:
 *
...
Ipv4IcmpNoRouteTestCase::ReceiveIcmp(Ptr<Socket> socket)
{
    ...
    if (ipv4.GetProtocol() != Icmpv4L4Protocol::PROT_NUMBER)
    {
        return;
    }
...
Ipv4IcmpNoRouteTestCase::DoRun()
{
    ...
    aIcmpSocket->SetAttribute("Protocol", UintegerValue(Icmpv4L4Protocol::PROT_NUMBER));
    ...
    aSendSocket->SetAttribute("Protocol", UintegerValue(Icmpv4L4Protocol::PROT_NUMBER));
    ...
    // Post-fix expectation: A receives exactly one ICMP Destination Unreachable
    // message, sourced from the router R (10.0.0.2).
    //
    // Pre-fix (current master): RouteInputError() drops the packet silently and
    // emits no ICMP, so m_icmpUnreachCount == 0 and this assertion FAILS. This
    // is the intended KNOWN-FAILING regression behaviour for issue #825.
    NS_TEST_EXPECT_MSG_EQ(m_icmpUnreachCount,
                          ...
                          "(issue #825). Pre-fix, 0 are received because "
                          "Ipv4L3Protocol::RouteInputError() drops silently.");
 ...
 * @ingroup tests
 *
 * @brief Test suite for the ICMP no-route regression (issue #825).
 *
 * KNOWN-FAILING: there is no fix yet; Ipv4L3Protocol::RouteInputError() does
 * not generate an ICMP net-unreachable message on a no-route forwarding
 * failure, so this suite is expected to fail on the current tree.
 */
