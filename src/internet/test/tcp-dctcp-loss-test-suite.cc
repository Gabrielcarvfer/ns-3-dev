 ...
 * @ingroup tests
 *
 * @brief KNOWN-FAILING regression test for GitLab issue #1012.
 *
 * Issue #1012: "DcTcp behaves incorrectly with non-ECN routers".
 *
 ...
 *
 * The current TcpDctcp::GetSsThresh implementation
 * (src/internet/model/tcp-dctcp.cc) ignores both @c bytesInFlight and the
 ...
 * @c m_ackedBytesEcn stays 0), so on a real (non-ECN) packet loss this
 * returns approximately @c m_cWnd, i.e. essentially NO reduction, instead
 * of the RFC-5681 loss-based reduction of approximately BytesInFlight / 2.
 *
 ...
 *
 * On current master this assertion is EXPECTED TO FAIL: GetSsThresh returns
 * ~cwnd (unchanged) rather than ~bytesInFlight/2. The failure IS the point
 * of this regression test; there is no fix for issue #1012 yet.
 */
...
{
    // Craft a sender state representing the moment a real (non-ECN) loss is
    ...
    // Mark this as a real loss event (as opposed to an ECN-CE reduction), so a
    ...
    // For a real (inferred) loss, RFC 8257 Section 3.5 requires the same
    ...
    NS_LOG_INFO("cWnd=" << m_cWnd << " bytesInFlight=" << m_bytesInFlight
                        << " GetSsThresh=" << newSsThresh
                        << " expected(~half)=" << expectedSsThresh);

    // KNOWN-FAILING on current master: with alpha == 0, GetSsThresh returns
    // ~cWnd (no reduction), so this assertion fails. Once issue #1012 is
    // fixed so DCTCP applies RFC-5681 loss-based backoff on non-ECN loss,
    // GetSsThresh should return ~bytesInFlight/2 and this assertion passes.
    NS_TEST_ASSERT_MSG_EQ_TOL(
        newSsThresh,
        expectedSsThresh,
        tolerance,
        "DCTCP did not apply RFC-5681 loss-based backoff on a real (non-ECN) "
 ...
 * @ingroup tests
 *
 * @brief TCP DCTCP non-ECN loss back-off TestSuite (regression for issue #1012).
 *
 * This suite is KNOWN-FAILING on current master: it documents that DCTCP does
 * not perform RFC-5681 loss-based congestion-window reduction on a real
 * (non-ECN-signalled) packet loss. See TcpDctcpLossSsThreshTest.
 */
  ...
  public:
        ...
        : TestSuite("tcp-dctcp-loss", Type::UNIT)
    {
        ...
        AddTestCase(new TcpDctcpLossSsThreshTest(20 * 1446,
                                                 ...
                                                 "real (non-ECN) loss with alpha=0"),
