...
class TcpHeaderMalformedOptionsTestCase : public TestCase
{
  public:
    ...
    void DoRun() override
    {
        ...
        // A duplicate option (two window scale options) would be shadowed by
        // the first instance when looked up with GetOption (see @issueid{940})
        NS_TEST_ASSERT_MSG_EQ(
            Deserialize({0x03, 0x03, 0x07, 0x03, 0x03, 0x07, 0x01, 0x00}).IsMalformed(),
            true,
            "A duplicate option was accepted");
    }
