 ...
 * @ingroup tests
 *
 * @brief KNOWN-FAILING regression test for issue #125.
 *
 * Issue #125: when the receiving application reads buffered data and thereby
 * frees up receive-buffer space, TCP does not proactively send a window-update
 * ACK. The only relief for a sender that has stalled on a zero (closed) window
 * is its own persist timer (zero-window probe). This is asymmetric with
 * TcpSocketBase::SetRcvBufSize(), which, when the buffer is enlarged while the
 * connection is established, explicitly transmits an ACK to advertise the
 * reopened window (see TcpSocketBase::SetRcvBufSize() in
 * src/internet/model/tcp-socket-base.cc). TcpSocketBase::Recv() (and the
 * Extract() it calls) frees the same buffer space but emits no such ACK.
 *
 * Scenario:
 ...
 *
 * Observed behaviour on the current tree: no ACK is sent in response to the
 * application read; the receiver's advertised window stays at 0 until the
 * sender's persist timer eventually probes. Therefore the assertion in
 * FinalChecks() that a window update was sent shortly after the read FAILS.
 *
 * This test asserts the CORRECT behaviour, so it FAILS on the current tree and
 * is expected to PASS once issue #125 is fixed.
 */
...
TcpWindowUpdateOnReadTest::FinalChecks()
{
    ...
    // CORRECT behaviour: reading data that frees receive-buffer space must elicit
    // a window-update ACK without waiting for the sender persist timer.
    // On the current tree no such ACK is sent, so this assertion FAILS (issue #125).
 ...
 * @ingroup tests
 *
 * @brief TCP window-update-on-read TestSuite (KNOWN-FAILING, issue #125).
 */
