 ...
 * @ingroup lte-test
 *
 * @brief Regression test for issue #447: a UE handing over between cells must
 ...
 * after the reconfiguration, leaving LteUePhy::GenerateMixedCqiReport()
 * dividing two SpectrumValue objects with different spectrum models. Before
 * the fix this aborted the simulation early (around +2.4 s in this scenario).
 *
 ...
 * completion. The simulation reaching its stop time is the pass condition,
 * because the bug manifests as a fatal assertion well before then.
 */
...
LteHandoverSpectrumModelTestCase::DoRun()
{
    ...
    // spectrum-model mismatch assertion aborting it (issue #447).
