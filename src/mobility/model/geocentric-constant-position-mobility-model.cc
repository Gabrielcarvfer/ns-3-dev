    ...
    Ptr<const GeocentricConstantPositionMobilityModel> other)
{
    ...
    // approximated by the distance from the Earth's centre: using the ECEF
    // z-coordinate was wrong, as z is the distance from the equatorial plane, not
    // the altitude (issue #1308).
    ...
    // sign must be preserved (the previous std::abs forced a positive angle even
    // for below-horizon terminals).
