...
while reducing cache misses. With this approach, significant speedup
can be achieved for large topologies with heavy traffic.
...
examples by omitting the ``-mtp`` in the program name)::
