...
// Forward declaration in the global namespace (matches the definition in
// model/sls-chan-wgpu.h, which is not in ns3:: for now). The destructor of
// ThreeGppChannelModel must be defined out-of-line so std::unique_ptr<>'s
// generated deleter sees the complete type there.
class SlsChanWgpu;
...
class ThreeGppChannelModel : public MatrixBasedChannelModel
{
  public:
    ...
    /**
     * @brief Force-refresh every dirty link in the channel-params cache in
     * one GPU batch.
     *
     * When the `UseGpu` attribute is true, this method walks
     * `m_channelParamsMap`, finds every link whose params would normally be
     * regenerated by `NewChannelParamsNeeded` or `ChannelUpdateNeeded` (i.e.
     * first-call, condition change, displacement > 1 m, or update period
     * elapsed), gathers their inputs from the per-link MobilityModel /
     * PhasedArrayModel / ChannelCondition state, and runs the WebGPU
     * SlsChanWgpu pipeline once for the whole batch. The kernel outputs
     * are then written back into the cache in exactly the shape
     * `GetNewChannel` expects, so downstream code (and especially
     * `ThreeGppSpectrumPropagationLossModel`) does not see the difference.
     *
     * Safe to call once per `Simulator::Now()` tick — internally
     * short-circuits if the batch already ran this time-step and no links
     * are dirty.
     *
     * No-op when `UseGpu` is false (the default for the moment) — every
     * link is then regenerated per-call by the CPU path as before.
     */
  ...
  protected:
    ...
    /**
     * @brief Per-link endpoint snapshot kept alive between `GetChannel`
     * calls so the batch back-end (see `EnsureBatchFresh`) can find the
     * mobility / antenna for every link it needs to refresh.
     *
     * The CPU path already has these inputs in scope inside `GetChannel`,
     * but `EnsureBatchFresh` runs *before* the per-link calls, so without
     * a side-table it has no way to look them up. We stash them here on
     * every `GetChannel` so subsequent ticks can refresh existing links
     * without round-tripping through the simulator scheduler.
     */
    struct LinkEndpoints
    {
        Ptr<const MobilityModel> aMob;       //!< Mobility of the A endpoint.
        Ptr<const MobilityModel> bMob;       //!< Mobility of the B endpoint.
        Ptr<const PhasedArrayModel> aAntenna;//!< Antenna of the A endpoint.
        Ptr<const PhasedArrayModel> bAntenna;//!< Antenna of the B endpoint.
    };

    /**
     * @brief Walk `m_channelParamsMap` and return the keys whose params
     * would be regenerated or updated by `GetChannel` right now.
     *
     * Uses the same predicates (`NewChannelParamsNeeded`,
     * `ChannelUpdateNeeded`) as the CPU path so the GPU batch refreshes
     * exactly the set the CPU path would have refreshed. Endpoints are
     * looked up via `m_linkEndpoints`; entries with no recorded
     * endpoints are skipped (they were never reached by a `GetChannel`
     * call and therefore can't be in `m_channelParamsMap` anyway).
     *
     * @return Vector of dirty link keys; empty if nothing is stale.
     */
    ...
    /**
     * Side-table populated on every `GetChannel` call so
     * `EnsureBatchFresh` can find the mobility / antenna of every link
     * it needs to refresh. Keyed by the same reciprocal `(nodeA,nodeB)`
    ...
    /// Use the WebGPU batch back-end instead of regenerating per link on
    /// the CPU. See EnsureBatchFresh() for details. Toggled by the
    /// `UseGpu` Attribute; default false (CPU path) until the integration
    /// passes parity tests.
    bool m_useGpu;
    ...
    /// where the complete type is visible to unique_ptr.
    std::unique_ptr<::SlsChanWgpu> m_gpu;
