    ...
    bool isReverse) const
{
    ...
    if (channelParams->m_cachedDelaySincos.size() < numRb)
    {
        channelParams->m_cachedDelaySincos =
            std::vector<std::valarray<std::complex<double>>>(numRb);
        auto sbit = inPsd->ConstBandsBegin(); // band iterator
        for (unsigned i = 0; i < numRb; i++)
        {
            double fsb = (sbit + i)->fc; // center frequency of the sub-band
            channelParams->m_cachedDelaySincos[i] = std::valarray<std::complex<double>>(numCluster);
    ...
    for (unsigned iRb = 0; iRb < numRb; iRb++)
    {
        if (*(vit+iRb) != 0.00)
        {
